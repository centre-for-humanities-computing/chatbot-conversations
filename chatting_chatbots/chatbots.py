...
class Experiment:
    ...
    batch_size=128, train_after_run=True, output_path="outputs", cache_path=os.path.join(".cache", "huggingface", "transformers")):
        ...
        for participant in self.population:
            ...
            if isinstance(self.population[participant], str):
                ...
                model = AutoModelForCausalLM.from_pretrained(self.population[participant], pad_token_id=tokenizer.eos_token_id, cache_dir=cache_path).to(self.device)
            ...
            # otherwise the tokenizer and model need to be passed
                ...
                self.models[participant][1] = self.population[participant][1].to(self.device)
    ...
    def train_participant(self, participant):
        ...
        data_collator = DataCollatorForLanguageModeling(self.models[participant][0], mlm=False)
